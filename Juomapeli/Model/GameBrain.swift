...
struct GameBrain {
    ...
    init(player1: String, player2: String) {
        ...
        self.tasks = [
            ...
            "\(player1), anna 30 sekunnin jalkahieronta pelaajalle \(player2) tai ota 4 huikkaa",
            "\(player1), yritä laittaa pelaajan \(player2) nyrkki omaan suuhusi tai juo 4 huikkaa",
            "\(player1), yritä nuolaista kyynärpäätäsi. Jos epäonnistut, juo 2 huikkaa",
            "\(player1), kuori banaani varpaillasi tai ota 4 huikkaa",
            "\(player1), puhu kolmen kierroksen ajan ilman, että huulesi koskettavat. Jos epäonnistut, juo 3 huikkaa",
            "\(player1), imitoi jotain julkkista kunnes muut pelaajat arvaavat ketä esität. Jos epäonnistut, juo 4 huikkaa",
            "\(player1), mene kolmen kierroksen ajaksi nurkkaan istumaan itseksesi tai juo 5 huikkaa",
            "\(player1), anna paras Hessu Hopo -imitaatiosi tai juo 4 huikkaa",
            "\(player1), riisu 2 valitsemaasi vaatekappaletta tai juo 4 huikkaa",
            "\(player1), yritä saada muut pelaajat nauramaan puhumatta. Jos epäonnistut, juo 3 huikkaa",
            "\(player1), jos puhelimesi ruutuaika on suurempi kuin pelaajalla \(player2), juo 3 huikkaa. Muussa tapauksessa \(player2) juo 3 huikkaa.",
            ...
            "\(player1), saat puhua vain kuiskaten seuraavan 3 kierroksen ajan. Jos epäonnistut, juo 3 huikkaa",
            "\(player1), twerkkaa 15 sekunnin ajan tai juo 4 huikkaa",
            "\(player1), näytä orgasmi-ilmeesi tai juo 4 huikkaa",
            "\(player1), syö banaani viettelevästi tai juo 3 huikkaa",
            "\(player1), soita ilmakitaraa 15 sekunnin ajan tai juo 4 huikkaa",
            ...
            "\(player1), ota 4 huikkaa",
            "\(player1), ota 2 huikkaa",
            "\(player1), ota 3 huikkaa",
            "\(player1), paras kehonosa vastakkaisella sukupuolella?",
            "\(player1), ota kivi-sakset-paperi -matsi pelaajan \(player2) kanssa. Häviäjä juo 3 huikkaa",
            "Kaikki pelaajat, joilla on siitin, juovat 3 huikkaa",
            "\(player1), jos olisit peruna, tulisitko mieluummin kuorituksi vai keitetyksi?",
            "\(player1), monenko ihmisen kanssa olet harrastanut seksiä? Vastaa tai juo 4 huikkaa",
            "\(player1), onko koolla väliä?",
            "\(player1), oletko omasta mielestäsi hyvä sängyssä? Vastaa tai juo 4 huikkaa",
            "\(player1), mikä on suurin fantasiasi? Vastaa tai juo 4 huikkaa",
            "\(player1), mikä on kiusallisin tilanne, johon olet joutunut? Vastaa tai juo 3 huikkaa",
            "\(player1), jos sinun pitäisi harrastaa roolileikkiä pelaajan \(player2) kanssa, miksi pukeutuisitte?",
            "\(player1), mikä on erikoisin paikka, jossa olet harrastanut seksiä? Vastaa tai juo 4 huikkaa"
        ]
